...
class RoutinePersistenceAdapter implements DailyRoutineRepositoryPort, RoutineItemRepositoryPort, WeatherSnapshotRepositoryPort {
    ...
    @Override public WeatherSnapshot save(WeatherSnapshot w){WeatherSnapshotJpaEntity e=weatherSnapshotJpaRepository.save(new WeatherSnapshotJpaEntity(w.id(),w.userId(),w.targetDate(),w.regionSido(),w.regionSigungu(),w.latitude(),w.longitude(),w.temperature(),w.uvIndex(),w.weatherCondition(),w.provider(),w.observedAt(),w.createdAt()));return new WeatherSnapshot(e.getId(),e.getUserId(),e.getTargetDate(),e.getRegionSido(),e.getRegionSigungu(),e.getLatitude(),e.getLongitude(),e.getTemperature(),e.getUvIndex(),e.getWeatherCondition(),e.getProvider(),e.getObservedAt(),e.getCreatedAt());}
