...
class NotificationDeliveryServiceTest {
    ...
    void sendsExpectedPayloadAndMarksLogSent() {
        ...
        then(sender).should().send("token-1", RoutineReadyNotificationTemplate.TITLE,
    ...
    void permanentProviderFailureMarksFailedAndDeactivatesOnlyThatDevice() {
        ...
        given(sender.send(eq("valid-token"), any(), any(), anyMap()))
                .willReturn(PushNotificationPort.DeliveryResult.sent());
    ...
    void transactionFailureForOneDeviceDoesNotBlockAnotherDevice() {
        ...
        given(sender.send(eq("second-token"), any(), any(), anyMap()))
                .willReturn(PushNotificationPort.DeliveryResult.sent());
    ...
    private PushDevice device(Long id, String token) {
        return new PushDevice(id, 1L, token, PushPlatform.WEB, true, NOW, NOW, NOW);
    }
